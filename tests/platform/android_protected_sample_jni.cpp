...
constexpr const char *kSmokeClassName = "com/vmp/smoke/ProtectedSmokeActivity";
...
std::uint32_t readU32(std::size_t &offset) {
  ensure(offset + 4 <= kProtectedSampleBlobSize, "truncated protected sample u32");
...
std::uint64_t readU64(std::size_t &offset) {
  ensure(offset + 8 <= kProtectedSampleBlobSize, "truncated protected sample u64");
...
Artifact parseEmbeddedArtifact() {
  ...
  ensure(opcodeCount == artifact.map.encode.size(), "unexpected protected sample opcode map size");
  for (std::size_t i = 0; i < artifact.map.encode.size(); ++i) {
    ensure(offset < kProtectedSampleBlobSize, "truncated protected sample opcode map");
  ...
  ensure(offset + payloadSize == kProtectedSampleBlobSize, "unexpected protected sample payload length");
...
static jint nativeProtectedAdd(JNIEnv *env, jobject self, jint lhs, jint rhs) {
  (void)env;
  (void)self;
  return (jint)vmp_platform_protected_add((int)lhs, (int)rhs);
}

static jint nativeProbePlatform(JNIEnv *env, jobject self) {
  (void)env;
  (void)self;
  return (jint)vmp_platform_probe().platform;
}

static jint nativeVerifyEmbeddedSample(JNIEnv *env, jobject self) {
  (void)env;
  (void)self;
  try {
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  jclass smokeClass = env->FindClass(kSmokeClassName);
  if (smokeClass == nullptr) {
    return JNI_ERR;
  }

  JNINativeMethod methods[] = {
      {"a", "(II)I", reinterpret_cast<void *>(nativeProtectedAdd)},
      {"b", "()I", reinterpret_cast<void *>(nativeProbePlatform)},
      {"c", "()I", reinterpret_cast<void *>(nativeVerifyEmbeddedSample)},
  };
