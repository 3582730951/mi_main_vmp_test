...
Artifact parseEmbeddedArtifact() {
  ...
  ensure(kProtectedSampleBlobSize >= expected.size(), "artifact too short");
  ensure(std::equal(expected.begin(), expected.end(), kProtectedSampleBlob), "invalid artifact magic");
...
int main() {
  try {
    ...
    std::cout << "release protected sample passed: " << passed << '\n';
    return 0;
  } catch (...) {
    return 40;
  }
